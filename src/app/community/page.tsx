...
import { MessageCircle, PenLine, ShieldCheck } from "lucide-react";
...
export default async function CommunityPage({ searchParams }: { searchParams: Promise<{ page?: string; category?: string }> }) {
  ...
  return <DashboardShell title={t("community.title")} eyebrow="Community" description={t("community.description")}>
    <section className="panel overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2 overflow-x-auto pb-1">{categories.map(([value,label]) => <Link key={value || "all"} href={value ? `/community?category=${value}` : "/community"} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-black ${category === value || (!category && !value) ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}>{label}</Link>)}</div><div className="flex shrink-0 items-center gap-2">{isAdmin ? <Link href="/community/write" className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-black text-white transition hover:bg-blue-500"><ShieldCheck size={16}/>SHadmin으로 글쓰기</Link> : null}<Link href="/community/write" className="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-black text-white"><PenLine size={16}/>{t("community.write")}</Link></div></div>
