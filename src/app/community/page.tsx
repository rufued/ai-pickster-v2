...
export default async function CommunityPage({ searchParams }: { searchParams: Promise<{ page?: string; category?: string }> }) {
  ...
  return <DashboardShell title={t("community.title")} eyebrow="Community" description={t("community.description")}>
    <section className="panel overflow-hidden">
      ...
      {posts.length ? <div className="divide-y divide-slate-100">{posts.map((post) => <Link href={`/community/${post.id}`} key={post.id} className="grid gap-2 px-4 py-4 transition hover:bg-slate-50 sm:grid-cols-[110px_minmax(0,1fr)_150px_145px] sm:items-center sm:px-5"><span className="w-fit rounded-full bg-blue-50 px-2.5 py-1 text-xs font-black text-blue-700">{categoryLabel(post.category as CommunityCategory)}</span><div className="min-w-0"><p className="break-words font-black text-slate-900">{post.title}</p><p className="mt-1 text-xs font-medium text-slate-500 sm:hidden">{post.nickname} · <LocalDateTime value={post.created_at} mode="mobile" /></p></div><span className="hidden truncate text-sm font-bold text-slate-600 sm:block">{post.nickname}</span><span className="flex items-center gap-3 text-xs font-bold text-slate-500"><span className="hidden sm:inline"><LocalDateTime value={post.created_at} mode="mobile" /></span><span className="inline-flex items-center gap-1"><MessageCircle size={14}/>{post.comment_count}</span></span></Link>)}</div> : <div className="p-12 text-center"><p className="font-black text-slate-800">{t("community.empty")}</p><p className="mt-2 text-sm text-slate-500">{t("community.emptyDesc")}</p></div>}
