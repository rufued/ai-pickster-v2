...
export default async function AiHistoryPage({ params }: { params: Promise<{ ai: string }> }) {
  ...
  return <main className="container-shell py-8"><AiPill aiId={aiId}/><h1 className="mt-3 text-3xl font-black text-slate-950">{ai.name} 정산 기록</h1><div className="mt-6 divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 bg-white">{rows.map((bet) => <article key={bet.id} className="grid gap-2 p-4 sm:grid-cols-[1fr_auto] sm:items-center"><span className="text-sm font-bold text-slate-800"><TeamMatchup homeTeam={bet.legs[0]?.homeTeam ?? ""} awayTeam={bet.legs[0]?.awayTeam ?? ""} compact /></span><span className={bet.profit >= 0 ? "font-black text-emerald-600" : "font-black text-red-600"}>{signedCurrency(bet.profit)}</span></article>)}</div>{rows.length === 0 ? <div className="mt-6 rounded-xl border border-slate-200 bg-white p-10 text-center text-sm font-bold text-slate-500">정산된 픽이 없습니다.</div> : null}</main>;
}
