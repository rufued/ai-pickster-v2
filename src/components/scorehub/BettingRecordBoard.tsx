  ...
  return (
    <article className="w-full min-w-0 max-w-full overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="grid gap-3 lg:grid-cols-[150px_minmax(0,1fr)_minmax(140px,auto)_80px_110px_110px] lg:items-center">
        ...
        <div className="text-xs font-bold text-slate-500">{leg ? <LeagueBadge league={leg.league} className="py-0.5 text-[11px]" /> : <p>-</p>}<p className="mt-1 text-[11px] font-black text-blue-700">{leg ? marketLabel(leg.market, t) : "-"}</p><p className="mt-1 text-sm font-black text-slate-900">{t("games.odds")} {bet.totalOdds.toFixed(2)}</p>{leg?.startTime ? <p className="mt-1 flex items-center gap-1 text-[11px] font-bold text-slate-400"><Clock3 size={11} />{t("picks.gameStart")} <LocalDateTime value={leg.startTime} mode="mobile" /></p> : null}</div>
...
function ParlayLeg({ leg, index }: { leg: BetLeg; index: number }) {
  ...
  return (
    <div className="grid gap-3 p-4 sm:grid-cols-[28px_minmax(0,1fr)_100px_80px] sm:items-center">
      ...
      <div className="min-w-0"><p className="flex flex-wrap items-center gap-1.5 text-[11px] font-bold text-slate-400"><LeagueBadge league={leg.league} className="py-0.5 text-[11px]" /><span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-600">{marketLabel(leg.market, t)}</span>{leg.startTime ? <span className="inline-flex items-center gap-1"><Clock3 size={11} />{t("picks.gameStart")} <LocalDateTime value={leg.startTime} mode="mobile" /></span> : null}</p><p className="mt-1 text-sm font-black text-slate-950"><TeamMatchup homeTeam={leg.homeTeam} awayTeam={leg.awayTeam} selectedSide={leg.selectedSide} pickType={leg.pickType} oddsOptions={leg.oddsOptions} compact /></p><p className="mt-1 whitespace-normal text-xs font-black text-blue-700 [overflow-wrap:anywhere]">{t("common.pick")}: {leg.selection}</p></div>
